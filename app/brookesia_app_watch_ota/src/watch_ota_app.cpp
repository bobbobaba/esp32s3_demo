...
static constexpr const char *LOGIN_URL = "http://<OTA_SERVER>/DUDUSERVER/api/v1/auth/login";
static constexpr const char *DEVICE_CONFIG_URL = "http://<OTA_SERVER>/DUDUSERVER/api/v1/device-config";
static constexpr const char *DEVICE_STATUS_URL = "http://<OTA_SERVER>/DUDUSERVER/api/v1/device-status";
static constexpr const char *USERNAME = "admin";
static constexpr const char *PASSWORD = "<OTA_PASSWORD>";
...
static constexpr const char *DETAIL_PATH = "/ota/page/info/detail/value";
...
std::string ota_state_text(OtaState state)
{
    switch (state) {
    ...
    case OtaState::Switching: return "switching";
...
std::string make_ota_detail_text(std::string_view state, std::string_view target, std::string_view partition)
{
    std::string detail = "state=" + std::string(state);
    detail += " target=";
    detail += target.empty() ? "--" : std::string(target);
    detail += " partition=";
    detail += partition.empty() ? "--" : std::string(partition);
    return detail;
}
...
std::expected<void, std::string> WatchOtaApp::on_start(system::core::AppContext &context)
{
    ...
    add_binding(updates, DETAIL_PATH, "value", "等待检测");
...
{
    {
        ...
        impl_->ota_state = operation == Operation::Check ? OtaState::Checking : OtaState::Downloading;
    }

    std::vector<gui::BindingValueUpdate> updates;
    add_binding(updates, STATUS_PATH, "status", operation == Operation::Check ? "正在检测更新..." : "准备 OTA 更新...");
    add_binding(updates, DETAIL_PATH, "value", make_ota_detail_text(
        ota_state_text(operation == Operation::Check ? OtaState::Checking : OtaState::Downloading),
        "",
        ""
    ));
    ...
    if (ok != pdPASS) {
        ...
        add_binding(failure_updates, DETAIL_PATH, "value", make_ota_detail_text("failed", "", ""));
...
void WatchOtaApp::run_operation(Operation operation)
{
    ...
    auto push_detail = [&push_updates](std::string_view state, std::string_view target, std::string_view partition) {
        std::vector<gui::BindingValueUpdate> updates;
        add_binding(updates, DETAIL_PATH, "value", make_ota_detail_text(state, target, partition));
        push_updates(updates);
    };
    ...
    if (!wifi_connected()) {
        {
            std::lock_guard lock(impl_->mutex);
            impl_->operation_running = false;
            impl_->ota_state = OtaState::Failed;
        }
        std::vector<gui::BindingValueUpdate> updates;
        add_binding(updates, STATUS_PATH, "status", "Wi-Fi 未连接，请先进入设置配网");
        add_binding(updates, DETAIL_PATH, "value", make_ota_detail_text("failed", "", ""));
    ...
    if (operation == Operation::Check) {
        ...
        push_detail(ota_state_text(OtaState::Checking), "", "");
        OtaConfig config;
        int status_code = 0;
        if (!fetch_config_with_retry(config, token, status_code)) {
            ...
            push_detail(ota_state_text(OtaState::Failed), "", "");
            finish_buttons();
            return;
        }
        ...
        {
            std::lock_guard lock(impl_->mutex);
            impl_->ota_state = available ? OtaState::Ready : OtaState::Idle;
        }
        std::vector<gui::BindingValueUpdate> updates;
        ...
        add_binding(updates, DETAIL_PATH, "value", make_ota_detail_text(
            available ? ota_state_text(OtaState::Ready) : ota_state_text(OtaState::Idle),
    ...
    if (!firmware_url_supported(config.firmware_url)) {
        ...
        push_detail(ota_state_text(OtaState::Failed), config.firmware_version, running_partition_label());
        finish_buttons();
        return;
    }
    ...
    {
        std::lock_guard lock(impl_->mutex);
        impl_->ota_state = OtaState::Downloading;
    }
    report_status(token, "OTA开始", config.firmware_version, target_partition_label, 0, 0, -1);
    {
        ...
        add_binding(updates, DETAIL_PATH, "value", make_ota_detail_text(
            ota_state_text(OtaState::Downloading),
            config.firmware_version,
    ...
    esp_http_client_handle_t http_client = esp_http_client_init(&http_config);
        ...
        push_detail(ota_state_text(OtaState::Failed), config.firmware_version, target_partition_label);
        finish_buttons();
        return;
    }
    ...
    {
        std::lock_guard lock(impl_->mutex);
        impl_->ota_state = OtaState::Verifying;
    }
    ...
    {
        std::lock_guard lock(impl_->mutex);
        impl_->ota_state = OtaState::Switching;
    }
    ...
    {
        std::lock_guard lock(impl_->mutex);
        impl_->ota_state = OtaState::Completed;
    }
    report_status(token, "OTA完成", config.firmware_version, target_partition_label);
    {
        ...
        add_binding(updates, WRITE_BAR_PATH, "value", "100");
        add_binding(updates, DETAIL_PATH, "value", make_ota_detail_text(
            ota_state_text(OtaState::Completed),
            config.firmware_version,
            target_partition_label
        ));
        add_button_state_bindings(updates, false, false);
