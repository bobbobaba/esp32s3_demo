...
constexpr const char *DEVICE_STATUS_URL = "<PRIVATE_OTA_BASE_URL>/api/v1/device-status";
constexpr const char *DEVICE_CONFIG_URL = "<PRIVATE_OTA_BASE_URL>/api/v1/device-config";
constexpr const char *WEATHER_URL = "<PRIVATE_OTA_BASE_URL>/api/v1/weather?compact=true";
constexpr const char *USERNAME = "<PRIVATE_USER>";
constexpr const char *PASSWORD = "<PRIVATE_PASSWORD>";
...
std::string login_token()
{
    ...
    const char *body = "{\"username\":\"admin\",\"password\":\"<PRIVATE_PASSWORD>\"}";
...
bool CloudApp::run(void)
{
    ...
    _detail_label = make_label(detail_card, "Cloud monitor reads private OTA backend status.", &lv_font_montserrat_16, 0xD7DCE5);
...
bool CloudApp::startRefresh()
{
    ...
    if (_mutex != nullptr && xSemaphoreTake(_mutex, pdMS_TO_TICKS(50)) == pdTRUE) {
        ...
        _running = true;
        _status = "Loading";
        _summary = "Contacting private OTA backend";
...
void CloudApp::cloudTask(void *arg)
{
    ...
    if (token.empty()) {
        app->setCloudState(false, false, "Error", "Login failed", "Check private OTA backend network/login.");
        vTaskDeleteWithCaps(nullptr);
        return;
    }
