...
constexpr const char *USAGE_URL = "<PRIVATE_OTA_BASE_URL>/api/v1/api-usage-status";
...
bool QuotaApp::run(void)
{
    ...
    _detail_label = make_label(detail_card, "API usage is synced through private OTA backend.", &lv_font_montserrat_14, 0xAEB7C6);
...
void QuotaApp::quotaTask(void *arg)
{
    ...
    if (token.empty()) {
        app->setState(false, false, "NO DATA", "Login failed", "Check private OTA backend login/network.", "Today: --", "Total: --", "Models: --", "");
        vTaskDeleteWithCaps(nullptr);
        return;
    }
    HttpResponse response;
