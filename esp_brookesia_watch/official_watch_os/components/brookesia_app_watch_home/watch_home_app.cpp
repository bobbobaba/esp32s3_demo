...
constexpr uint32_t HOME_STATUS_REFRESH_MS = 5000;
...
bool WatchHomeApp::run(void)
{
    ...
    lv_label_set_text(brand, "WatchHome");
...
void WatchHomeApp::refreshStatus()
{
    if (_battery_label != nullptr) {
        ...
        lv_label_set_text(_battery_label, battery_text);
    }
    ...
    if (!watch::wifi_is_connected() && watch::wifi_has_credentials()) {
        if (_wifi_reconnect_tick == 0 || lv_tick_elaps(_wifi_reconnect_tick) > 30000) {
    ...
    if (_weather_label != nullptr) {
        ...
        const bool stale = has_snapshot && snapshot.valid &&
            ...
            (esp_timer_get_time() - snapshot.last_refresh_us) > 10LL * 60LL * 1000000LL;
        ...
        if (has_snapshot && snapshot.valid) {
            ...
            lv_label_set_text(_weather_label, weather_text);
        } else if (watch::weather_is_running()) {
            lv_label_set_text(_weather_label, "weather loading...");
        } else if (!watch::wifi_is_connected()) {
            lv_label_set_text(_weather_label, "weather offline");
        } else {
            lv_label_set_text(_weather_label, "weather --");
        }
    }
    ...
    if (_anniv_label != nullptr) {
        ...
        lv_label_set_text(_anniv_label, anniv_text);
    }

    if (_quota_label != nullptr) {
        if (watch::wifi_is_connected() && !watch::quota_refresh_is_running()) {
            const bool should_refresh = _quota_refresh_tick == 0 || lv_tick_elaps(_quota_refresh_tick) > 5 * 60 * 1000;
            if (should_refresh) {
        ...
        lv_label_set_text(_quota_label, quota_text);
    }
}
