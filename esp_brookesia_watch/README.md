...
- WiFi 配网/扫描继续加固：保留已修复的 URL 编码解析，自动连接和扫描前增加 internal heap largest block 检查，低内存时直接提示而不是继续创建任务。
- 表盘首页修复 LVGL timer 生命周期：进入首页前清理旧 timer，返回菜单或关闭首页时删除 timer，并清空页面对象指针，降低反复切换首页/菜单后的卡屏和残留刷新风险。
- OTA App 防重复任务：OTA task 未完全退出时阻止再次点击启动，下载/校验/切换分区期间继续禁用按钮，降低重复任务造成的状态错乱。
- Music App 防重入：云列表/下载任务运行时禁止同时启动本地播放，本地播放任务运行时禁止同时启动下载；播放完成后重新扫描本地音乐列表。
- Files App 增加轻量子入口：`SD Music`、`SD Logs`、`LittleFS`，便于直接查看音乐目录、日志目录和 LittleFS 根目录状态。
- 本版本仍只更新 app 固件；不刷 NVS / LittleFS / SD / otadata，保留 WiFi 记录、设置项、已下载 App 和 SD 内容。
- 已本地构建通过并上传 OTA：固件 ID `142`；app 分区余量约 65%；固件镜像版本头确认为 `0.1.64`；远端 bin 与本地 release SHA256 一致。

0.1.63 新增：

- 修复 `xTaskCreateWithCaps()` 后台任务退出时未使用 `vTaskDeleteWithCaps()` 的问题，覆盖 Settings WiFi、WiFi 自动连接/DNS/配网守护、Weather、Quota、Cloud、OTA、Music、Sensors 等任务，降低反复进入 App 后 no memory、闪退和重启风险。
- 首页主时间取消 `transform_zoom` 缩放绘制，改为原生字号/固定宽度布局，降低圆角屏上横纹、脏刷新和裁切风险。
- OTA 启动上报任务启动时调用 `esp_ota_mark_app_valid_cancel_rollback()`，为后续启用 OTA 回滚机制预留安全确认点。
- 本版本仍只更新 app 固件；不刷 NVS / LittleFS / SD / otadata，保留 WiFi 记录、设置项、已下载 App 和 SD 内容。
- 已本地构建通过：app 分区余量约 65%；固件镜像版本头确认为 `0.1.63`。

0.1.62 新增：

- 启动后自动上报当前固件版本、运行分区和 boot 分区，方便云端确认线刷/OTA 后到底运行哪个版本。
- OTA App 页面和状态上报增加 `Boot` 分区信息，和 `Run / Next` 一起显示，定位启动槽问题更直接。
- Settings `About` / `System Health` 增加 `Run / Boot / Next` 分区显示。
- Settings 非忙时刷新周期从 3 秒改为 5 秒，降低页面常驻时的 UI 和后台压力。
- 修复 Settings `About` 卡片重复注册问题，并调整 Sensor / Audio / About 卡片高度，减少内容裁切。
- 首页 WiFi 状态更明确：无连接但有保存网络时显示 `WiFi saved`，没有保存网络时显示 `WiFi off`。
- 首页天气离线时显示 `weather offline`，不再显示容易误解的占位状态。
- Music 本地播放前先检查 MP3/WAV 文件头；无效文件直接提示原因，不进入播放任务。
- Music 本地播放任务改用 PSRAM task，并在 task 创建失败时显示 internal heap / largest block，降低播放闪退概率。
- 本版本不改 NVS / LittleFS / SD / otadata，不清 WiFi 记录、设置项和已下载 App。

0.1.61 新增：

- 修复 OTA 末尾可观测性和可靠性：将下载 buffer 从 OTA task 栈移动到堆内存，buffer 从 8KB 降到 4KB，task 栈从 24KB 降到 20KB。
- OTA 末尾状态拆分为 `OTA校验开始`、`OTA校验完成`、`OTA切换启动分区`、`OTA完成，准备重启`，便于判断到底卡在 `esp_ota_end()`、`esp_ota_set_boot_partition()` 还是重启后启动槽。
- `esp_ota_set_boot_partition()` 后读取 boot partition 做一次确认，不一致时上报失败，不直接重启。
- 本版本不改 NVS / LittleFS / SD / otadata，不清 WiFi 记录、设置项和已下载 App。
...
- 已本地构建通过：app 分区余量约 65%；已生成 release，待上传 private OTA backend 后可 OTA。
...
- 新增 `Cloud` 云服务器状态监控 App：后台登录 private OTA backend，读取设备在线状态、固件/OTA、内存/RSSI、天气摘要和云端目标版本。
- 新增 `Quota` API 额度监控 App：后台读取 private OTA backend `GET /api/v1/api-usage-status`，显示 provider、套餐、额度、已用比例、剩余额度、更新时间和备注。
