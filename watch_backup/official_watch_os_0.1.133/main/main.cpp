...
constexpr bool WATCH_ENABLE_POWER_MANAGEMENT = true;
...
constexpr const char *AI_CHAT_APP_NAME = "AI Chat";
...
constexpr TickType_t POWER_BUTTON_POLL_INTERVAL = pdMS_TO_TICKS(500);
...
void configure_power_management()
{
    ...
    if (err == ESP_OK) {
        ESP_UTILS_LOGI(
            "Power management enabled: DFS max=%dMHz min=%dMHz, light sleep off",
...
extern "C" void app_main(void)
{
    ...
    if constexpr (WATCH_ENABLE_POWER_MANAGEMENT) {
        ...
        ESP_UTILS_LOGW("Power management disabled by build policy");
    }
...

    {
        ...
        for (const auto &[name, app] : inited_apps) {
            ...
            } else if ((app != nullptr) && (std::strcmp(name.c_str(), AI_CHAT_APP_NAME) == 0)) {
                ai_chat_app_id = app->getId();
            }
        }
        esp_brookesia::apps::watch_home_set_ai_chat_app_id(ai_chat_app_id);
        if (ai_chat_app_id < 0) {
            ESP_UTILS_LOGW("AI Chat app not found; WatchHome LiveTalk shortcut disabled");
        } else {
            ESP_UTILS_LOGI("WatchHome LiveTalk target: %s (id: %d)", AI_CHAT_APP_NAME, ai_chat_app_id);
        }
        ...
        }, WATCH_ENABLE_DISPLAY_TICK ? 15000 : 1000, phone);
