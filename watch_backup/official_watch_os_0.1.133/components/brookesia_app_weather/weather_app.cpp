...
lv_obj_t *make_label(lv_obj_t *parent, const char *text, const lv_font_t *font, uint32_t color)
{
    ...
    watch_display::set_label_text(label, text, &lv_font_montserrat_14);
...
void WeatherApp::refreshUi()
{
    ...
    if (!watch::wifi_is_connected()) {
        ...
        std::snprintf(detail, sizeof(detail), "Loading DUDUSERVER weather...");
        ...
        std::snprintf(sync, sizeof(sync), "DUDUSERVER weather");
    }

    if (_main_label != nullptr) {
        set_label_text_if_changed(_main_label, main);
        lv_obj_set_style_text_color(_main_label, lv_color_hex(ok ? 0xFFFFFF : (running ? 0xFFCC33 : 0xFFFFFF)), 0);
    }
    if (_detail_label != nullptr) {
        set_label_text_if_changed(_detail_label, detail);
    }
    if (_network_label != nullptr) {
        set_label_text_if_changed(_network_label, wifi);
    }
    if (_sync_label != nullptr) {
        set_label_text_if_changed(_sync_label, sync);
    }
    if (_refresh_label != nullptr) {
        ...
        set_label_text_if_changed(_refresh_label, running ? "Loading..." : (recent ? "OK" : "Refresh"));
    }
}
