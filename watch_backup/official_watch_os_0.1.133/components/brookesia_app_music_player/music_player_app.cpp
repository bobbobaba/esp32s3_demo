...
constexpr const char *MUSIC_LIST_URL = WATCH_PRIVATE_DUDUSERVER_API_BASE_URL "/music/list";
constexpr const char *PUBLIC_BASE_URL = WATCH_PRIVATE_SERVER_BASE_URL;
...
std::string login_token()
{
    ...
    constexpr const char *body = WATCH_PRIVATE_LOGIN_BODY;
...
void MusicPlayerApp::refresh()
{
    ...
    if (_player_title_label != nullptr) {
        if (has_cloud_selection) {
            watch_display::set_label_text(_player_title_label, _cloud_tracks[_current_cloud_index].title, &lv_font_montserrat_18);
        } else if (has_track) {
            watch_display::set_label_text(_player_title_label, display_title(_tracks[_current_index]), &lv_font_montserrat_18);
        } else {
