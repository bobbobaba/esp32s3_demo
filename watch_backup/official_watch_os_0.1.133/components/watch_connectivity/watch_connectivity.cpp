...
bool s_power_save_suspended = false;
...
{
    if (!s_initialized || !s_wifi_driver_initialized || !s_wifi_started || s_provisioning || s_power_save_suspended) {
        return ESP_OK;
    }
    if (!wifi_op_lock(pdMS_TO_TICKS(5000))) {
        ESP_LOGW(TAG, "power-save WiFi pause skipped: operation lock busy");
        return ESP_ERR_TIMEOUT;
    }
    ...
    if ((err == ESP_OK) || (err == ESP_ERR_WIFI_NOT_STARTED) || (err == ESP_ERR_WIFI_STATE)) {
        s_wifi_started = false;
        ESP_LOGI(TAG, "WiFi suspended while display is off");
        wifi_op_unlock();
        return ESP_OK;
    }
    s_power_save_suspended = false;
    ESP_LOGW(TAG, "power-save WiFi pause failed: %s", esp_err_to_name(err));
    wifi_op_unlock();
    return err;
}
...
{
    if (!s_initialized || !s_power_save_suspended) {
        return ESP_OK;
    }
    s_power_save_suspended = false;
    if (s_provisioning || !wifi_has_credentials()) {
        return ESP_OK;
    }
    s_manual_disconnect = false;
    s_last_error[0] = '\0';
    ESP_LOGI(TAG, "WiFi resume requested after display wake");
    return request_autoconnect(false);
}
...
void wifi_status_text(char *buffer, size_t buffer_size)
{
    ...
    } else if (s_power_save_suspended) {
        std::snprintf(buffer, buffer_size, "WiFi sleeping with display off");
    } else if (s_connected) {
...
void wifi_short_status_text(char *buffer, size_t buffer_size)
{
    ...
    if (s_connected) {
        ...
        std::snprintf(buffer, buffer_size, "WiFi sleep");
    } else if (s_wifi_state == WifiState::Failed) {
