...
constexpr uint32_t LOGIN_RETRY_DELAY_MS = 1200;
...
bool login(char *token, size_t token_size, HttpResponse *diagnostic = nullptr)
{
    HttpResponse response;
    const bool ok = http_request(HTTP_METHOD_POST, LOGIN_URL, "", WATCH_PRIVATE_LOGIN_BODY, response) &&
                    parse_token(response.body, token, token_size);
...
bool login_with_retry(char *token, size_t token_size, HttpResponse *diagnostic = nullptr)
{
    HttpResponse last_response;
    for (int attempt = 0; attempt < LOGIN_ATTEMPTS; ++attempt) {
        if (login(token, token_size, &last_response)) {
            if (diagnostic != nullptr) {
                *diagnostic = last_response;
            }
            return true;
        }
        if (attempt + 1 < LOGIN_ATTEMPTS) {
            vTaskDelay(pdMS_TO_TICKS(LOGIN_RETRY_DELAY_MS));
        }
    }
    if (diagnostic != nullptr) {
        *diagnostic = last_response;
...
void format_login_failure(char *buffer, size_t buffer_size, const HttpResponse &response)
{
    if (response.status >= 200 && response.status < 300 && response.error == ESP_OK) {
        std::snprintf(buffer, buffer_size, "invalid login reply");
    } else if (response.status > 0) {
        std::snprintf(buffer, buffer_size, "HTTP %d", response.status);
    } else if (response.error != ESP_OK) {
        std::snprintf(buffer, buffer_size, "network %s", esp_err_to_name(response.error));
    } else {
        std::snprintf(buffer, buffer_size, "no server response");
    }
}
...
void startup_report_task(void *)
{
    ...
    for (int attempt = 0; attempt < 8; ++attempt) {
        ...
        char token[384] = {};
        if (!login_with_retry(token, sizeof(token))) {
            continue;
        }
...
void OtaApp::flushUiState()
{
    ...
    if (_status_label != nullptr) {
        watch_display::set_label_text(_status_label, status, &lv_font_montserrat_14);
    }
    ...
    if (_detail_label != nullptr) {
        watch_display::set_label_text(_detail_label, detail, &lv_font_montserrat_12);
    }
    if (_log_label != nullptr) {
        watch_display::set_label_text(_log_label, (has_config && config.log[0] != '\0') ? config.log : CONFIG_URL, &lv_font_montserrat_12);
    }
...
void OtaApp::runOperation(Operation operation)
{
    ...
    if (_token[0] == '\0') {
        ...
        if (!login_with_retry(_token, sizeof(_token), &login_response)) {
            char reason[96] = {};
            format_login_failure(reason, sizeof(reason), login_response);
            char status[128] = {};
            char detail[160] = {};
            std::snprintf(status, sizeof(status), "OTA login failed: %s", reason);
            std::snprintf(detail, sizeof(detail), "State: failed / login after %d tries / WiFi %d dBm", LOGIN_ATTEMPTS, watch::wifi_rssi_dbm());
            setStatus(status);
            setDetail(detail);
            _update_available = false;
            finish();
    ...
    if (!fetch_config(_token, config, &status)) {
        if (status == 401 || status == 403) {
            _token[0] = '\0';
            if (login_with_retry(_token, sizeof(_token))) {
