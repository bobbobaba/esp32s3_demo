...
void quota_home_text(char *buffer, size_t buffer_size)
{
    ...
    const bool has_tokens = snapshot.today_tokens > 0.0 || snapshot.total_tokens > 0.0;
    const bool has_cost = snapshot.today_cost > 0.0 || snapshot.total_cost > 0.0;
    const bool has_balance = snapshot.balance > 0.0;
    const bool has_provider = snapshot.provider[0] != '\0' && std::strcmp(snapshot.provider, "Quota") != 0;
    const bool usage_only = std::strcmp(snapshot.status, "usage_only") == 0;
    const bool waiting_for_usage = std::strcmp(snapshot.status, "not_synced") == 0;
    if (!has_tokens && !has_cost && !has_balance) {
        if (has_provider && (usage_only || waiting_for_usage)) {
            std::snprintf(buffer, buffer_size, "%s active", snapshot.provider);
        } else {
    ...
    if (usage_only) {
        if (has_cost) {
            std::snprintf(buffer, buffer_size, "%s D %s%.2f", snapshot.provider, symbol, snapshot.today_cost);
        } else {
            std::snprintf(buffer, buffer_size, "%s U %s tok", snapshot.provider, today_tokens);
        }
    } else if (snapshot.balance <= 0.0 && snapshot.total_cost > 0.0) {
        std::snprintf(buffer, buffer_size, "%s U %s%.2f", snapshot.provider, symbol, snapshot.total_cost);
    } else if (has_tokens) {
        std::snprintf(
            buffer,
            buffer_size,
