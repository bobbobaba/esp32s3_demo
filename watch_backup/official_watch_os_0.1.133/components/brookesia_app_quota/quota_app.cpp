...
constexpr const char *USAGE_URL = WATCH_PRIVATE_DUDUSERVER_API_BASE_URL "/api-usage-status";
...
void QuotaApp::refreshUi()
{
    ...
    if (_headline_label != nullptr) {
        set_label_text_if_changed(_headline_label, headline.c_str());
    }
    if (_title_label != nullptr) {
        set_label_text_if_changed(_title_label, title.c_str());
        lv_obj_set_style_text_color(_title_label, lv_color_hex(ok ? 0x23D18B : (running ? 0xFFCC33 : 0xFFFFFF)), 0);
    }
    if (_summary_label != nullptr) {
        set_label_text_if_changed(_summary_label, summary.c_str());
    }
    if (_today_label != nullptr) {
        set_label_text_if_changed(_today_label, today.c_str());
    }
    if (_total_label != nullptr) {
        set_label_text_if_changed(_total_label, total.c_str());
    }
    if (_models_label != nullptr) {
        set_label_text_if_changed(_models_label, models.c_str());
    }
    if (_detail_label != nullptr) {
        set_label_text_if_changed(_detail_label, detail.c_str());
    }
    if (_refresh_label != nullptr) {
        const bool recent = last_us > 0 && (esp_timer_get_time() - last_us) < 3000000;
        set_label_text_if_changed(_refresh_label, running ? "Loading..." : (recent ? (ok ? "OK" : "Retry") : "Refresh"));
    }
}
...
void QuotaApp::quotaTask(void *arg)
{
    ...
    const bool has_active_provider = !provider.empty() && provider != "-" && provider != "Quota";
    const bool usage_only = status == "usage_only";
    const bool provider_ready_without_usage = status == "not_synced" && has_active_provider;
    if (!has_metric && status != "unlimited" && !usage_only && !provider_ready_without_usage) {
        char detail[384] = {};
        std::snprintf(
            detail,
            sizeof(detail),
    ...
    char headline[48] = {};
    char title[48] = {};
    if (usage_only || provider_ready_without_usage) {
        std::snprintf(headline, sizeof(headline), "ACTIVE PROVIDER");
        std::snprintf(title, sizeof(title), "%s", provider.c_str());
    ...
    char summary[128] = {};
    if (usage_only) {
        std::snprintf(summary, sizeof(summary), "%s - usage tracked, quota unavailable", provider.c_str());
    } else if (provider_ready_without_usage) {
        std::snprintf(summary, sizeof(summary), "%s - no usage logged yet", provider.c_str());
    } else if (balance_only) {
    ...
    if (usage_only || provider_ready_without_usage) {
        std::snprintf(
            ...
            "Active provider: %s\nQuota: %s\nSource: %s\nAccount: %s\nPlan: %s\nStatus: %s\nMonth cost: %s\nLatest: %s\nUpdated: %s",
            provider.c_str(),
            usage_only ? "not provided by provider" : "waiting for local usage",
            source.c_str(),
            account.c_str(),
            plan.c_str(),
            status.c_str(),
    ...
    const bool ok = status != "error" && (status != "not_synced" || provider_ready_without_usage);
    if (ok) {
