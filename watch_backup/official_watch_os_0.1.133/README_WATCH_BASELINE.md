...
- 工程版本：`0.1.133`
- 本快照包含水平仪 App 及其角度跳变修复；详细版本资产和校验记录见
  [`RELEASE_HISTORY.md`](RELEASE_HISTORY.md)。
- GitHub 脱敏快照不包含 `build/`、`managed_components/`、固件文件、生成的
  `sdkconfig` 或私有配置头文件。
...
- 版本：`0.1.133`
- 大小：二进制大小 `0x4a7bd0`（4,881,360 bytes）
- SHA256：`c083d1bf31f5c0faba4c8fa0815c1c6bc93591d0c6a135bd7b36aeef19e1900d`
- OTA app 分区：11 MB
- 余量：约 38%
...
## 2026-08-16 LiveTalk / 中文字体未发布修复

- 中文显示不再依赖手工追加少量字符。`watch_display/lv_font_watch_zh_14.c`
  由工程内的 `NotoSansSC-Regular.subset.ttf` 重新生成，覆盖
  `U+4E00-U+9FFF`、全角标点和常用符号（约 5512 个 CJK 字符），
  `么`、`吗`、`们`、`？`、`，` 等不再落到方框替代字形。
- LiveTalk 音频不再等待整段 PCM 收完才播放。手表端使用 PSRAM
  StreamBuffer，累计约 32 KiB（约 0.7 秒）后启动独立 DMA 播放任务；
  播放和 WebSocket 接收并行，保留 160 ms 预缓冲以避免首字爆音/卡顿。
- LiveTalk 服务端只取最近 4 条对话，并对语音回答增加“一句话、最多 36 个汉字”的
  专用约束，减少模型和 TTS 的无效输出。
- 本次仅更新应用固件代码，不修改 NVS、WiFi、LittleFS、otadata 或分区表。
- 本地验证构建：`build/esp-brookesia.bin`，SHA256：
  `1428715e2f3f0892948091eebf906cae1173cddec9329e608761b5e4a27f0481`
  （尚未作为正式版本发布）。
...
- Weather：天气 App，通过云端天气接口读取真实天气；联网请求在后台 task 中执行，UI 只读缓存。无网刷新不会清空上次真实天气缓存。
- Timer：秒表/倒计时 App，支持开始/暂停、重置、模式切换、倒计时 +/-1 分钟和到点提示音。
- Cloud：云服务器状态监控 App，读取 DUDUSERVER 设备状态、天气、设备配置摘要，以及服务器 CPU、内存、磁盘、GPU 可用性和关键 systemd 服务状态。
...
## 2026-08-13 0.1.79 settings performance / storage async

## 2026-08-13 0.1.80 diagnostics async

## 2026-08-13 0.1.81 gallery / media folders

- 新增 Gallery 应用，读取真实 SD 卡状态和文件列表；
- 新增 `/sdcard/photos` 与 `/sdcard/videos` 标准目录；
- Photos/Videos 列表读取在低优先级后台任务执行，页面切换不会阻塞 LVGL；
- 当前固件未启用 JPEG/PNG 解码，也没有视频解码显示链路，页面明确提示不可预览/播放，不显示假内容；
- 设置页后台任务增加页面代次保护，SD 异常自动重试采用更长退避；
- 版本：工程版本从 `0.1.80` 提升到 `0.1.81`；

## 2026-08-13 0.1.81 cloud gallery / cloud video

- Gallery 新增 `Cloud Photos` 与 `Cloud Videos` 入口；
- 云端列表通过现有 DUDUSERVER 登录接口和媒体 API 获取，网络请求放在后台任务，避免阻塞 LVGL；
- 云照片使用服务器生成的 RGB565 设备下载地址，云视频使用服务器生成的 ESP32 帧包地址；
- 本次已上传测试媒体：`watch-photo-456.jpg`（照片 ID 4）和 `watch-video-4537.mp4`（视频 ID 3）；视频已生成 45 帧 ESP32 帧包；
- 当前 Gallery 显示云端清单和设备下载链接；JPEG/MP4 原始预览及视频播放仍需后续接入 RGB565/RGB565V 显示管线，不伪造预览内容；
- 本次构建 SHA256：`919d3247c3912bc4cc9d57db7215f8c502b2f941af432eaace10ead4bc7481c7`；
- 构建：已本地构建通过，应用分区剩余 48%；
- Release：`releases/0.1.81-watch-os-brookesia.bin`；
- SHA256：`644d5b063f7677e98095834c3d58f407d574bc57a5303b867ab12765ac2e1bd7`；
- OTA：本次未上传，待实机线刷验证后再决定是否推送。

- Settings / Health：Health 大段诊断文本移到低优先级后台任务，LVGL 线程只提交缓存结果。
- Settings / Sensor：IMU I2C 读取移到低优先级后台任务，避免 I2C 异常阻塞设置页。
- Settings / Refresh：进入 Health/Sensor 页面立即后台刷新，之后按 15 秒节流；手动 Refresh 仍立即触发。
- Settings / Power：手动刷新按钮会立即刷新电池/电源诊断。
- 构建：已本地构建通过，应用分区剩余 48%。
- Release：`releases/0.1.80-watch-os-brookesia.bin`
- SHA256：`171b67bcd13324bbb6b142da8d603ab82d4d58ec2baf5d7e45fa7347f0aefb88`
- OTA：本次未上传，待实机验证后再决定是否推送。

- WatchHome：底部标识改为从 `esp_app_desc.version` 读取的 `v版本号`，当前显示 `v0.1.80`，后续构建自动同步；
- Settings / Storage：
  - TF 卡格式化改为低优先级后台任务，双击确认后界面不再被卸载、挂载、格式化操作阻塞；
  - LittleFS / TF 状态查询后台任务降为低优先级，并在两项查询之间主动让出调度；
  - 存储页自动重试间隔从 15 秒放宽到 60 秒，手动 Refresh 仍立即执行；
  - Storage 结果继续通过 dirty 标记回到 LVGL 线程更新，保留错误状态颜色提示；
- Settings / Display：
  - 熄屏时间滑块拖动时只更新预览文本，松手或触摸丢失时才提交一次 NVS，减少卡顿和闪存磨损；
- Settings / Diagnostics：
  - 电池、电源、Health、Sensor 诊断刷新节流到 15 秒，避免页面定时器频繁进行 I2C/大文本重排；
- 版本：工程版本从 `0.1.78` 提升到 `0.1.79`。
- 构建：已本地构建通过，应用分区剩余 48%。
- Release：`releases/0.1.79-watch-os-brookesia.bin`
- SHA256：`75a34e7589280a1b89ca51e294434effe3136c8dec64af7dbfa677865ff48d79`
- OTA：本次未上传，待实机线刷验证后再决定是否推送。

## 2026-08-13 0.1.78 non-WiFi polish / docs sync

- 本版本按“第 2 项 WiFi/配网暂不做”的要求，未修改 WiFi 扫描、二维码配网、键盘输入、保存网络、自动切换等连接逻辑。
- Version / README：
  - 工程版本从 `0.1.77` 提升到 `0.1.78`；
  - 修正 README 顶部仍写 `0.1.67` 的版本不一致问题；
  - 修正 Weather 功能描述，当前已接云端天气接口，不再写成占位。
- WatchHome / Weather：
  - 天气刷新在无 WiFi 时不再清空上次真实天气缓存；
  - 首页仍按真实缓存显示天气；没有真实缓存时显示 `weather --` / `weather offline`，不显示假天气。
- WatchHome / Quota：
  - Quota 首页摘要不再要求 token 数大于 0 才显示；
  - 当云端只提供真实费用/余额、没有 token 字段时，首页显示真实金额摘要，而不是误显示 `Quota --`。
- Settings / Storage：
  - Storage 子页周期刷新 LittleFS / TF 卡状态，便于插卡、挂载、格式化后观察状态变化；
  - 不自动格式化 LittleFS，不影响 AppStore 下载应用和资源。
- OTA / 启动上报：
  - 继续保持启动 OTA status report 关闭，避免恢复之前 0.1.75 一类白屏风险。
- 构建：已本地构建通过，固件镜像版本头确认为 `0.1.78`。
- Release：`releases/0.1.78-watch-os-brookesia.bin`
- SHA256：`527e70bb7eba92b7bd2417a82568022f6ad54a07454e80df4ee7ff37066c6ad0`
- OTA：本次未上传，需实机构建验证后再决定是否推送。
...
- 本版本仍只更新 app 固件；不刷 NVS / LittleFS / SD / otadata，保留 WiFi 记录、设置项和 AppStore 下载内容。
- 已上传并推送 OTA：固件 ID `137`，URL `http://<OTA_SERVER>/DUDUSERVER/firmware/0.1.59-50b5952490664acd8ddff5690fbcc8bf.bin`，远端 bin 与本地 release SHA256 一致。
...
### 0.1.119 LiveTalk WebSocket audio framing fix

- 修复 LiveTalk 音频分片被丢弃的问题：
  - ESP-IDF WebSocket 可能把一个带 FIN 的 JSON 帧拆成多次 `esp_transport_read()`；
  - 手表端现在按声明的 WebSocket payload 长度累积后再解析 JSON，不会把半个 `audio.delta` 当作坏 JSON 丢弃；
  - 增加 Base64 解码失败和部分帧诊断日志。
- 云端 duplex fallback 将 PCM 分片限制为 768 字节，降低 TCP 分片概率。
- 不擦除 NVS、Wi‑Fi、LittleFS、otadata 或 SD 卡。

### Gallery / media capability

Added in firmware `0.1.81` (build SHA256:
`9ccf694e61cfa481270c9051dfa2ea06e83855b7901aef3d2641bd1c4002af72`).

The current firmware includes a low-risk `Gallery` app. It lists real files from
`/sdcard/photos` and `/sdcard/videos` and reports the SD mount state. JPEG/PNG
preview and video playback remain explicitly disabled because this build does
not enable an LVGL image decoder or a video decode/display pipeline; the app
does not fabricate thumbnails or playback state.
