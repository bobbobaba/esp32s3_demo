...
constexpr const char *MUSIC_LIST_URL = "http://<OTA_SERVER>/DUDUSERVER/api/v1/music/list";
constexpr const char *PUBLIC_BASE_URL = "http://<OTA_SERVER>";
...
std::string login_token()
{
    ...
    constexpr const char *body = "{\"username\":\"admin\",\"password\":\"<OTA_PASSWORD>\"}";
...
std::string normalize_url(const std::string &url)
{
    ...
    return std::string(PUBLIC_BASE_URL) + "/DUDUSERVER/" + url;
}
...
void MusicPlayerApp::startLocalPlay(int index)
{
    ...
    if (xTaskCreate(playTaskEntry, "music_local_play", 12288, this, 4, &_play_task) != pdPASS) {
        _play_task = nullptr;
        setCloudState(false, 0, "Play task create failed");
    }
}
...
void MusicPlayerApp::runCloudFetch()
{
    ...
    auto *parsed = static_cast<CloudMusicTrack *>(
        heap_caps_calloc(max_parsed_tracks, sizeof(CloudMusicTrack), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT)
    );
    if (parsed == nullptr) {
        parsed = static_cast<CloudMusicTrack *>(std::calloc(max_parsed_tracks, sizeof(CloudMusicTrack)));
    }
    if (parsed == nullptr) {
        cJSON_Delete(root);
    ...
    cJSON_ArrayForEach(item, tracks) {
        ...
        if (url.empty() || (url == std::string(PUBLIC_BASE_URL) + "/DUDUSERVER/")) {
            continue;
        }
