...
constexpr const char *CONFIG_URL = "http://<OTA_SERVER>/DUDUSERVER/api/v1/device-config";
constexpr const char *STATUS_URL = "http://<OTA_SERVER>/DUDUSERVER/api/v1/device-status";
...
constexpr size_t OTA_BUFFER_SIZE = 8 * 1024;
constexpr uint32_t OTA_TASK_STACK_SIZE = 24 * 1024;
...
{
    return "Run " + partition_label(esp_ota_get_running_partition()) + " / Next " + partition_label(esp_ota_get_next_update_partition(nullptr));
}
...
void OtaApp::startOperation(Operation operation)
{
    ...
    if (task_ok != pdPASS) {
        ...
        setDetail("State: failed / create task");
        setButtons(true, _update_available);
        flushUiState();
    }
}
...
void OtaApp::runOperation(Operation operation)
{
    ...
    std::array<uint8_t, OTA_BUFFER_SIZE> buffer = {};
    ...
    setDetail(dl_detail);
    while (true) {
        int read = esp_http_client_read(client, reinterpret_cast<char *>(buffer.data()), buffer.size());
        if (read < 0) {
            err = ESP_FAIL;
            break;
        }
        ...
        if (!ota_started) {
            ...
            std::memcpy(header.data() + header_used, buffer.data(), copy);
            header_used += copy;
            ...
            if (copy < static_cast<size_t>(read)) {
                err = esp_ota_write(handle, buffer.data() + copy, static_cast<size_t>(read) - copy);
                if (err != ESP_OK) {
                    break;
                }
            }
        ...
        if (!chunk_written) {
            err = esp_ota_write(handle, buffer.data(), read);
            if (err != ESP_OK) {
                break;
            }
        }
    ...
    report_status(_token, "OTA校验并切换启动分区", config.version, target_partition.c_str(), 100, total_read, content_length);
    ...
    report_status(_token, "OTA完成", config.version, target_partition.c_str(), 100, total_read, content_length);
