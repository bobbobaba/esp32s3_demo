...
- 版本：`0.1.77`
- 大小：约 3.9 MB，二进制大小 `0x3dd6f0`
- SHA256：`01e3caffea121ced6ab74c845d5382f8213d4373dad3a5e015a945a1d3d64541`
- OTA app 分区：11 MB
- 余量：约 48%
...
- AI Chat：手表端语音/文字 AI 对话；普通 TTS 回复使用 WAV 解包 + PCM 预缓冲播放，降低整文件下载等待和中途断流卡顿。
...
## 2026-08-12 0.1.77 delayed WiFi reconnect / safe startup

- 保持 0.1.76 安全启动基线：不启用 OTA 启动上报。
- 0.1.75 已验证会触发白屏，原因锁定为开机早期 OTA startup report 任务；后续不得在开机早期恢复该任务。
- 首页启动后延迟 8 秒，如果已保存 WiFi 且未连接，则触发一次 `wifi_reconnect_saved()`；原 30 秒自动重试保留。
- 目标是改善启动后偶发无网，同时避免 WiFi/HTTP 任务抢占 LVGL 和首页初始化资源。
- 线刷验证：只刷 app 分区 `0x20000`，不擦 NVS / LittleFS / SD / otadata / bootloader。
- Release：`releases/0.1.77-watch-os-brookesia.bin`（本地保留，不提交 GitHub）
- SHA256：`01e3caffea121ced6ab74c845d5382f8213d4373dad3a5e015a945a1d3d64541`
