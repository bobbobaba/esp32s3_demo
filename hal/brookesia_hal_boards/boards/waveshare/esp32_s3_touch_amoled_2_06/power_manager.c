...
esp_err_t power_manager_get_battery_state(void *device_handle, power_manager_battery_state_t *state)
{
    ...
    *state = (power_manager_battery_state_t) {
    ...
    /*
     * Keep runtime battery polling read-only on ESP32-S3-Touch-AMOLED-2.06.
     *
     * Earlier black-screen failures were caused by applying AXP2101 settings
     * copied from other Waveshare boards. Even non-rail PMIC writes are avoided
     * here during automatic status polling; the Settings app can show voltage
     * only when the board/bootloader has already enabled those ADC channels.
     */
...
int power_manager_init(void *config, int cfg_size, void **device_handle)
{
    ...
    /*
     * ESP32-S3-Touch-AMOLED-2.06 has board-specific PMIC power rail wiring.
     *
     * Do not reuse the 2.16-inch board's destructive AXP2101 initialization here:
     * writing DC/LDO enable and voltage registers can turn off rails used by the
     * display after the boot logo is already visible. For this board the power
     * manager is intentionally limited to attaching the I2C device and reading
     * battery/USB status. Charger configuration is only changed if an explicit
     * higher-level caller invokes the setter APIs.
     */
    uint8_t status1 = 0;
    err = axp2101_read_reg(handle->pm_handle, AXP2101_REG_STATUS1, &status1);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to read AXP2101 status register");
        goto cleanup;
    }
    ESP_LOGI(TAG, "AXP2101 detected, status1=0x%02x; preserving existing board power rails", status1);
