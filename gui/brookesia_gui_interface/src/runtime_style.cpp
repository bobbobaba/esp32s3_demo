...
ResolvedStyle Runtime::Impl::resolve_style(const TreeRecord &tree, const Node &node) const
{
    ...
    auto resolve_font_chain = [&](std::string_view requested_font_id, bool allow_builtin_fallback = false) {
        ...
        if ((tree.environment.language == "zh_CN" || tree.environment.language == "zh-CN") &&
                (font_id == "default" || font_id == "zh_CN")) {
            BROOKESIA_LOGD(
                "Using backend built-in watch Chinese font instead of runtime file font: requested_font_id='%1%'",
                font_id
            );
            return;
        }
