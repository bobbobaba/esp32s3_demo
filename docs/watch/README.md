...
- 最新 release：`releases/0.1.77-watch-os-brookesia.bin`（本地 release，不提交 GitHub）
- 最新 SHA256：`01e3caffea121ced6ab74c845d5382f8213d4373dad3a5e015a945a1d3d64541`
- 最新 OTA 固件 ID：待上传服务器后记录
- 最新 OTA URL：待上传服务器后记录

0.1.77 新增：

- 继续保持 0.1.76 安全基线：不启用 OTA 启动上报，避免 0.1.75 复现白屏。
- 首页启动后延迟 8 秒，如果已有保存 WiFi 且尚未连接，则触发一次自动重连；原 30 秒重试逻辑保留。
- 用于改善启动后偶发无网的问题，避免在 LVGL/首页未稳定前抢 WiFi/HTTP 资源。
- 本版本只刷 app 分区，不刷 NVS / LittleFS / SD / otadata / bootloader。
- GitHub 只同步脱敏源码备份，不提交 release `.bin`，不暴露服务器地址、IP 或默认密码。
...
- 已本地构建通过：app 分区余量约 65%；已生成 release，待上传 DUDUSERVER 后可 OTA。

0.1.42 新增：

- 新增 `Cloud` 云服务器状态监控 App：后台登录 DUDUSERVER，读取设备在线状态、固件/OTA、内存/RSSI、天气摘要和云端目标版本。
- 新增 `Quota` API 额度监控 App：后台读取 DUDUSERVER `GET /api/v1/api-usage-status`，显示 provider、套餐、额度、已用比例、剩余额度、更新时间和备注。
...
- `AI Chat`：手表端语音/文字 AI 对话 App；普通 TTS 回复使用 WAV 解包 + PCM 预缓冲播放，降低文字已出但语音迟到或中途卡顿的问题。
...
- 登录：`POST http://<OTA_SERVER>/DUDUSERVER/api/v1/auth/login`
- 配置：`GET http://<OTA_SERVER>/DUDUSERVER/api/v1/device-config`
- 状态：`PUT http://<OTA_SERVER>/DUDUSERVER/api/v1/device-status`
- 固件下载：`GET http://<OTA_SERVER>/DUDUSERVER/firmware/<firmware>.bin`

设备端限制：

- 当前只支持 `http://` 固件 URL，不支持 `https://`。
- Wi-Fi 未连接时，OTA App 会提示先进入 Settings 配网。
- 登录账号当前写在 App 代码里：`admin` / `<OTA_PASSWORD>`，后续如果要公开发布固件，需要改成设备绑定或安全 token 方案。
