...
- Cloud：云服务器状态监控 App，读取 private OTA server 设备状态、天气、设备配置摘要。
- Quota：API 额度监控 App，读取 private OTA server `api-usage-status` 摘要；本机 ccswitch 同步只上传脱敏后的余额/用量字段。
...
- 本版本仍只更新 app 固件；不刷 NVS / LittleFS / SD / otadata，保留 WiFi 记录、设置项和 AppStore 下载内容。
- 已上传并推送 OTA：固件 ID `137`，URL `<PRIVATE_OTA_FIRMWARE_URL>`，远端 bin 与本地 release SHA256 一致。
...
- private OTA server quota 同步接口拒绝空/默认零值；没有真实采集数据时保持 `not_synced`。
...
- 0.1.41 已上传并推送 OTA：固件 ID `122`，URL `<PRIVATE_OTA_FIRMWARE_URL>`，远端 bin 与本地 release SHA256 一致。
...
- 0.1.42 已上传并推送 OTA：固件 ID `123`，URL `<PRIVATE_OTA_FIRMWARE_URL>`，远端 bin 与本地 release SHA256 一致。
