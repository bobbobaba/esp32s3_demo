...
std::expected<void, std::string> System::close_active_app()
{
    ...
    if (!active || active->app_id == shell_app_id_) {
        pending_shell_page_ = ShellPage::Home;
        if (!shell_app_) {
            return {};
        }
        auto page_result = shell_app_->show_page(ShellPage::Home);
        if (!page_result) {
            return page_result;
        }
...
{
    return open_shell_page(ShellPage::Home);
}
...
std::expected<void, std::string> System::restore_shell()
{
    ...
    if (pending_shell_page_ == ShellPage::Home || shell_app_ == nullptr) {
        return {};
    }
