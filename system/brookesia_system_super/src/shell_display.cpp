...
std::expected<void, std::string> ShellApp::mount_overlay(core::AppContext &context)
{
    ...
    if (overlay_action_connections_.empty()) {
        auto subscribe_action = [this, &context](
                                    std::string_view action,
                                    std::function<void(const gui::Event &)> handler
                                ) -> std::expected<void, std::string> {
            auto connection = context.gui().subscribe_action(action, std::move(handler));
            if (!connection.connected())
            {
                return std::unexpected(std::string("Failed to subscribe Shell action: ") + std::string(action));
            }
            overlay_action_connections_.push_back(std::move(connection));
            return {};
        };
        if (auto result = subscribe_action(SUPER_ACTION_SYSTEM_UI_MASK_CLICK, [](const gui::Event &) {}); !result) {
            return result;
        }
        if (auto result = subscribe_action(SUPER_ACTION_SYSTEM_UI_MASK_GESTURE, [](const gui::Event &) {}); !result) {
            return result;
        }
        if (auto result = subscribe_action(SUPER_ACTION_OPEN_LAUNCHER, [this](const gui::Event &) {
                if (auto nav_result = owner_.open_app_launcher(); !nav_result) {
                    BROOKESIA_LOGW("Failed to open launcher from shell action: %1%", nav_result.error());
                }
            }); !result) {
            return result;
        }
        if (auto result = subscribe_action(SUPER_ACTION_HOME, [this](const gui::Event &) {
                if (auto nav_result = owner_.go_home(); !nav_result) {
                    BROOKESIA_LOGW("Failed to go home from shell action: %1%", nav_result.error());
                }
            }); !result) {
            return result;
        }
...
void ShellApp::handle_display_touch_gesture(const DisplayService::TouchGestureInfo &info)
{
    ...
    if (foreground_is_shell_) {
        if (current_shell_page_ == ShellPage::AppLauncher &&
                info.event_type == DisplayService::TouchGestureEventType::Pressing &&
                started_from_bottom) {
            const auto upward_delta = std::max(0, info.start_y - info.stop_y);
            const auto exit_distance = std::max<int32_t>(get_gesture_exit_distance_px(owner_.get_environment()) / 2, 36);
            if (upward_delta >= exit_distance &&
                    (info.direction == DisplayService::TouchGestureDirection::Up ||
                     info.direction == DisplayService::TouchGestureDirection::None)) {
                if (auto nav_result = owner_.go_home(); !nav_result) {
                    BROOKESIA_LOGW("Failed to go home from launcher bottom gesture: %1%", nav_result.error());
                }
            }
        }
        return;
