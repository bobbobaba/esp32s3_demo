...
std::expected<void, std::string> Backend::start_app(AppId id)
{
    ...
    // QuickJS module loading uses fopen() on LittleFS, which calls esp_partition_read().
    // ESP-IDF asserts if flash/cache operations run while the current task stack is
    // in PSRAM. SystemCore workers intentionally use PSRAM stacks to save SRAM, so
    // run the synchronous JS start path on a short-lived internal-stack thread.
    if (!lib_utils::ThreadConfig::check_stack_cache_safe()) {
        ...
        try {
            ...
            boost::thread start_thread([this, id, &result]() {
                try {
                    result = this->start_app(id);
                } catch (const std::exception &e) {
                    result = std::unexpected(std::string("JavaScript app start exception: ") + e.what());
                } catch (...) {
                    result = std::unexpected("JavaScript app start unknown exception");
                }
            });
            start_thread.join();
        } catch (const std::exception &e) {
            return std::unexpected(std::string("Failed to create internal-stack JavaScript start thread: ") + e.what());
        } catch (...) {
            return std::unexpected("Failed to create internal-stack JavaScript start thread");
        }
        return result;
    }
#endif
